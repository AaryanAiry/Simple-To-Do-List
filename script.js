function saveTasks() {
    ...
    document.querySelectorAll(".todo").forEach(li => {
        tasks.push({
            ...
            date: li.querySelector(".todo-date").innerText,
...
function loadTasks() {
    ...
    savedTasks.forEach(task => {
        addTask(task.text, task.date, task.checked);
    });
}
function addTask(taskText, taskDate = "", checked = false) {
    ...
    li.innerHTML = `
        ...
        <label class="custom-checkbox" for="${taskId}">
            <svg fill="transparent" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"><path d="m424-296 282-282-56-56-226 226-114-114-56 56 170 170Z"/></svg>
        </label>
    ...
    li.querySelector("input[type='checkbox']").addEventListener("change", saveTasks);
